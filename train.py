"""
Compatibility training entry point modeled after the upstream repository layout.

The active solver still lives in `maze_solver.py`. This script provides a
friendly runner for the older `train.py` workflow while using the current maze
environment, knowledge format, and renderers.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the upstream-style training entry point on the current solver.")
    ...
    parser.add_argument("--knowledge", default=None, help="Knowledge JSON path. Defaults to results/<maze-name>.json.")
    parser.add_argument("--viz-dir", default=None, help="Directory for legacy static episode snapshots.")
    parser.add_argument("--reset-knowledge", action="store_true", help="Ignore any existing knowledge file and start fresh.")
    parser.add_argument("--no-render", action="store_true", help="Skip path renders and legacy episode snapshots.")
...
def main():
    ...
    print("=" * 55)
    print("UPSTREAM-COMPATIBLE TRAIN RUN")
    print("=" * 55)
